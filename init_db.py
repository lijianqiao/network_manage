...
from app.db.connection import init_database, close_database, generate_schemas
...
async def init_database_with_sample_data():
    ...
    try:
        ...
        from app.models import (
            ...
            DeviceTypeEnum, DeviceStatusEnum, ConfigTemplate, TemplateTypeEnum
        )
        ...
        beijing_region = await Region.create(
            name="无锡区域", 
        ...
        # 创建品牌数据
        h3c_brand = await Brand.create(
            name="H3C",
            platform_type="h3c_comware",
            description="华三网络设备"
        )
        
        huawei_brand = await Brand.create(
            name="Huawei",
            platform_type="huawei_vrp", 
            description="华为网络设备"
        )
        
        cisco_brand = await Brand.create(
            name="Cisco",
        ...
        await DeviceModel.create(
            name="S5700",
            brand=h3c_brand,
            description="H3C S5700系列交换机"
        )
        
        await DeviceModel.create(
            name="CE12800",
            brand=huawei_brand,
            description="华为CE12800系列核心交换机"
        )
        
        await DeviceModel.create(
            name="Catalyst 9300",
            brand=cisco_brand,
            description="Cisco Catalyst 9300系列交换机"
        )
        
        # 创建设备分组
        core_group = await DeviceGroup.create(
            name="核心交换机",
            region=chengdu_region,
            description="核心网络设备"
        )
        
        access_group = await DeviceGroup.create(
            name="接入交换机",
            region=chengdu_region,
            description="接入层网络设备"
        )
        ...
        await ConfigTemplate.create(
            ...
            description="查询设备MAC地址表"
        )
        
        await ConfigTemplate.create(
            name="端口配置",
            template_type=TemplateTypeEnum.CONFIG,
            description="配置交换机端口"
        )
...
async def reset_database():
    ...
    try:
        ...
        # 导入所有模型
        from app.models import (
            RollbackOperation, ConfigDiff, ConfigSnapshot,
            DeviceConnectionStatus, OperationLog, TemplateCommand,
            ConfigTemplate, Device, DeviceGroup, DeviceModel,
            Brand, Region
        )
