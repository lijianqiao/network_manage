...
class RollbackOperationCreateRequest(BaseCreateSchema):
    ...
    rollback_reason: Optional[str] = Field(default=None, description="回滚原因")
...
class RollbackOperationUpdateRequest(BaseUpdateSchema):
    """回滚操作更新请求"""
    rollback_status: Optional[RollbackStatusEnum] = Field(default=None, description="回滚状态")
    executed_by: Optional[str] = Field(default=None, min_length=1, max_length=100, description="执行回滚的操作者")
    rollback_reason: Optional[str] = Field(default=None, description="回滚原因")
...
class RollbackOperationResponse(BaseResponseSchema):
    ...
    rollback_reason: Optional[str] = Field(description="回滚原因")
    
    # 关联信息
    device_id: Optional[UUID] = Field(default=None, description="设备ID")
    device_name: Optional[str] = Field(default=None, description="设备名称")
    device_ip: Optional[str] = Field(default=None, description="设备IP")
    original_operation_type: Optional[str] = Field(default=None, description="原始操作类型")
    snapshot_type: Optional[str] = Field(default=None, description="快照类型")
    
    # 执行统计
    rollback_duration: Optional[float] = Field(default=None, description="回滚耗时（秒）")
    rollback_attempts: Optional[int] = Field(default=None, description="回滚尝试次数")
...
class RollbackOperationDetailResponse(RollbackOperationResponse):
    """回滚操作详情响应"""
    # 包含详细的回滚过程信息
    rollback_log: Optional[List[Dict[str, Any]]] = Field(default=None, description="回滚日志")
    validation_results: Optional[Dict[str, Any]] = Field(default=None, description="验证结果")
    related_snapshots: Optional[List[Dict[str, Any]]] = Field(default=None, description="相关快照")


class RollbackOperationQueryParams(BaseQueryParams, TimeRangeQuery):
    """回滚操作查询参数"""
    original_operation_id: Optional[UUID] = Field(default=None, description="按原始操作ID筛选")
    target_snapshot_id: Optional[UUID] = Field(default=None, description="按目标快照ID筛选")
    rollback_status: Optional[RollbackStatusEnum] = Field(default=None, description="按回滚状态筛选")
    executed_by: Optional[str] = Field(default=None, description="按执行者筛选")
    device_id: Optional[UUID] = Field(default=None, description="按设备ID筛选")
    device_name: Optional[str] = Field(default=None, description="按设备名称筛选")
    region_id: Optional[UUID] = Field(default=None, description="按区域ID筛选")
    duration_min: Optional[float] = Field(default=None, ge=0, description="最小耗时（秒）")
    duration_max: Optional[float] = Field(default=None, ge=0, description="最大耗时（秒）")


class RollbackExecuteRequest(BaseCreateSchema):
    """回滚执行请求"""
    operation_log_id: UUID = Field(description="要回滚的操作记录ID")
    target_snapshot_id: Optional[UUID] = Field(default=None, description="指定目标快照ID（可选）")
...
class RollbackExecuteResponse(BaseResponseSchema):
    ...
    # 执行信息
    backup_snapshot_id: Optional[UUID] = Field(default=None, description="回滚前备份快照ID")
    target_snapshot_id: UUID = Field(description="目标快照ID")
    rollback_time: float = Field(description="回滚耗时（秒）")
    
    # 验证信息
    validation_passed: Optional[bool] = Field(default=None, description="验证是否通过")
    validation_details: Optional[Dict[str, Any]] = Field(default=None, description="验证详情")
    
    # 错误信息
    error_message: Optional[str] = Field(default=None, description="错误信息")
    rollback_log: Optional[List[str]] = Field(default=None, description="回滚日志")
...
class RollbackValidationResponse(BaseResponseSchema):
    ...
    # 验证结果
    connectivity_check: Optional[bool] = Field(default=None, description="连通性检查")
    config_integrity: Optional[bool] = Field(default=None, description="配置完整性")
    service_status: Optional[Dict[str, Any]] = Field(default=None, description="服务状态")
    
    # 详细信息
    validation_details: Dict[str, Any] = Field(description="验证详情")
    issues_found: Optional[List[Dict[str, Any]]] = Field(default=None, description="发现的问题")
...
class RollbackStatsResponse(BaseResponseSchema):
    ...
    most_rolled_back_device: Optional[Dict[str, Any]] = Field(default=None, description="回滚最多的设备")
...
class RollbackReportRequest(BaseCreateSchema):
    """回滚报告请求"""
    device_ids: Optional[List[UUID]] = Field(default=None, description="设备ID列表")
...
class RollbackReportResponse(BaseResponseSchema):
    ...
    # 导出信息
    export_url: Optional[str] = Field(default=None, description="导出文件URL")
