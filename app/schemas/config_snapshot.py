...
class ConfigSnapshotCreateRequest(BaseCreateSchema):
    ...
    operation_log_id: Optional[UUID] = Field(default=None, description="关联操作记录ID")
...
class ConfigSnapshotUpdateRequest(BaseUpdateSchema):
    """配置快照更新请求"""
    config_content: Optional[str] = Field(default=None, min_length=1, description="配置内容")
    operation_log_id: Optional[UUID] = Field(default=None, description="关联操作记录ID")
...
class ConfigSnapshotResponse(BaseResponseSchema):
    ...
    operation_log_id: Optional[UUID] = Field(description="关联操作记录ID")
    ...
    # 配置统计
    config_size: Optional[int] = Field(default=None, description="配置大小（字节）")
    line_count: Optional[int] = Field(default=None, description="配置行数")
...
class ConfigSnapshotListResponse(BaseResponseSchema):
    ...
    # 隐藏配置内容以提高性能
    config_preview: Optional[str] = Field(default=None, description="配置预览（前100字符）")


class ConfigSnapshotDetailResponse(ConfigSnapshotResponse):
    """配置快照详情响应"""
    # 包含相关的差异记录
    related_diffs: Optional[List[dict]] = Field(default=None, description="相关差异记录")
    rollback_operations: Optional[List[dict]] = Field(default=None, description="回滚操作记录")
...
class ConfigSnapshotQueryParams(BaseQueryParams, TimeRangeQuery):
    ...
    snapshot_type: Optional[SnapshotTypeEnum] = Field(default=None, description="按快照类型筛选")
    operation_log_id: Optional[UUID] = Field(default=None, description="按操作记录ID筛选")
    checksum: Optional[str] = Field(default=None, description="按校验码筛选")
    ...
    size_min: Optional[int] = Field(default=None, ge=0, description="最小配置大小")
    size_max: Optional[int] = Field(default=None, ge=0, description="最大配置大小")
...
class ConfigSnapshotBackupResponse(BaseResponseSchema):
    ...
    snapshot_id: Optional[UUID] = Field(description="生成的快照ID")
    backup_status: str = Field(description="备份状态")
    config_size: Optional[int] = Field(default=None, description="配置大小")
    backup_time: float = Field(description="备份耗时（秒）")
    error_message: Optional[str] = Field(default=None, description="错误信息")
    
    # 如果启用自动比较
    has_changes: Optional[bool] = Field(default=None, description="是否有变更")
...
class ConfigSnapshotRestoreResponse(BaseResponseSchema):
    ...
    backup_snapshot_id: Optional[UUID] = Field(default=None, description="恢复前备份快照ID")
...
class ConfigSnapshotCleanupRequest(BaseCreateSchema):
    """配置快照清理请求"""
    device_ids: Optional[List[UUID]] = Field(default=None, description="指定设备ID列表")
    older_than_days: Optional[int] = Field(default=None, ge=1, description="清理N天前的快照")
    keep_latest_count: Optional[int] = Field(default=None, ge=1, description="每设备保留最新N个快照")
    snapshot_types: Optional[List[SnapshotTypeEnum]] = Field(default=None, description="要清理的快照类型")
