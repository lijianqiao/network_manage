...
class ConfigTemplateResponse(BaseResponseSchema):
    ...
    # 统计信息
    command_count: int | None = Field(default=0, description="关联命令数量")
    supported_brands: list[str] | None = Field(default=None, description="支持的品牌列表")
    usage_count: int | None = Field(default=0, description="使用次数")
...
class ConfigTemplateDetailResponse(ConfigTemplateResponse):
    ...
    # 包含关联的模板命令
    template_commands: list[dict] | None = Field(default=None, description="模板命令列表")
    recent_operations: list[dict] | None = Field(default=None, description="最近使用记录")


class ConfigTemplateQueryParams(BaseQueryParams):
    """配置模板查询参数"""

    name: str | None = Field(default=None, description="按模板名称筛选")
    template_type: TemplateTypeEnum | None = Field(default=None, description="按模板类型筛选")
    is_active: bool | None = Field(default=None, description="按启用状态筛选")
    brand_id: UUID | None = Field(default=None, description="按支持品牌筛选")
...
class TemplateCommandQueryParams(BaseQueryParams):
    ...
    config_template_id: UUID | None = Field(default=None, description="按模板ID筛选")
    brand_id: UUID | None = Field(default=None, description="按品牌ID筛选")
    template_name: str | None = Field(default=None, description="按模板名称筛选")
...
class TemplateExecuteResponse(BaseResponseSchema):
    ...
    parsed_output: list[dict[str, Any]] | None = Field(default=None, description="解析后的输出")
