...
class DeviceConnectionStatusCreateRequest(BaseCreateSchema):
    ...
    last_success_time: datetime | None = Field(default=None, description="最后成功连接时间")
    failure_count: int = Field(default=0, ge=0, description="连续失败次数")
    failure_reason: str | None = Field(default=None, description="失败原因")
    snmp_response_time_ms: int | None = Field(default=None, ge=0, description="SNMP响应时间（毫秒）")


class DeviceConnectionStatusUpdateRequest(BaseUpdateSchema):
    """设备连接状态更新请求"""

    is_reachable: bool | None = Field(default=None, description="设备是否可达")
    last_check_time: datetime | None = Field(default=None, description="检查时间")
    last_success_time: datetime | None = Field(default=None, description="最后成功连接时间")
    failure_count: int | None = Field(default=None, ge=0, description="连续失败次数")
    failure_reason: str | None = Field(default=None, description="失败原因")
    snmp_response_time_ms: int | None = Field(default=None, ge=0, description="SNMP响应时间（毫秒）")
...
class DeviceConnectionStatusResponse(BaseResponseSchema):
    ...
    last_success_time: datetime | None = Field(description="最后成功连接时间")
    failure_count: int = Field(description="连续失败次数")
    failure_reason: str | None = Field(description="失败原因")
    snmp_response_time_ms: int | None = Field(description="SNMP响应时间（毫秒）")
    ...
    # 计算字段
    status_duration: int | None = Field(default=None, description="当前状态持续时间（分钟）")
    reliability_score: float | None = Field(default=None, description="可靠性评分")
...
class DeviceConnectionStatusQueryParams(BaseQueryParams, TimeRangeQuery):
    ...
    is_reachable: bool | None = Field(default=None, description="按可达性筛选")
    region_id: UUID | None = Field(default=None, description="按区域ID筛选")
    device_type: str | None = Field(default=None, description="按设备类型筛选")
    failure_count_min: int | None = Field(default=None, ge=0, description="最小失败次数")
    failure_count_max: int | None = Field(default=None, ge=0, description="最大失败次数")
    response_time_max: int | None = Field(default=None, ge=0, description="最大响应时间（毫秒）")
    offline_duration_min: int | None = Field(default=None, ge=0, description="最小离线时长（分钟）")
...
class ConnectionStatusStatsResponse(BaseResponseSchema):
    ...
    # 响应时间统计
    avg_response_time: float | None = Field(default=None, description="平均响应时间（毫秒）")
    max_response_time: int | None = Field(default=None, description="最大响应时间（毫秒）")
    min_response_time: int | None = Field(default=None, description="最小响应时间（毫秒）")
...
class ConnectionHealthCheckRequest(BaseCreateSchema):
    ...
    device_ids: list[UUID] | None = Field(default=None, description="指定设备ID列表（为空则检查所有）")
...
class ConnectionMonitoringConfigRequest(BaseCreateSchema):
    ...
    device_ids: list[UUID] | None = Field(default=None, description="监控设备列表（为空则监控所有）")
...
class ConnectionAlertResponse(BaseResponseSchema):
    ...
    failure_count: int = Field(description="连续失败次数")
    last_success_time: datetime | None = Field(description="最后成功时间")
...
class ConnectionTrendResponse(BaseResponseSchema):
    ...
    reachability_rate: float = Field(description="可达率")
    avg_response_time: float | None = Field(description="平均响应时间")
