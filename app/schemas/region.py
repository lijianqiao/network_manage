...
class RegionUpdateRequest(BaseUpdateSchema):
    ...
    snmp_community_string: str | None = Field(default=None, min_length=1, max_length=100, description="SNMP社区字符串")
    default_cli_username: str | None = Field(default=None, min_length=1, max_length=50, description="默认CLI账号")
...
class RegionResponse(BaseResponseSchema):
    ...
    device_group_count: int | None = Field(default=0, description="设备分组数量")
...
class RegionDetailResponse(RegionResponse):
    ...
    # 可以包含关联的设备分组等信息
    device_groups: list[dict] | None = Field(default=None, description="设备分组列表")
