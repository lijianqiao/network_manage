...
class OperationLogCreateRequest(BaseCreateSchema):
    """操作日志创建请求"""
    device_id: Optional[UUID] = Field(default=None, description="操作的设备ID")
    template_id: Optional[UUID] = Field(default=None, description="使用的模板ID")
    command_executed: Optional[str] = Field(default=None, description="执行的命令")
    output_received: Optional[str] = Field(default=None, description="设备返回的输出")
    parsed_output: Optional[Dict[str, Any]] = Field(default=None, description="解析后的输出")
    status: OperationStatusEnum = Field(description="操作状态")
    error_message: Optional[str] = Field(default=None, description="错误信息")
    executed_by: Optional[str] = Field(default=None, max_length=100, description="操作者")


class OperationLogResponse(BaseResponseSchema):
    """操作日志响应"""
    device_id: Optional[UUID] = Field(description="设备ID")
    template_id: Optional[UUID] = Field(description="模板ID")
    command_executed: Optional[str] = Field(description="执行的命令")
    output_received: Optional[str] = Field(description="设备输出")
    parsed_output: Optional[Dict[str, Any]] = Field(description="解析后输出")
    status: OperationStatusEnum = Field(description="操作状态")
    error_message: Optional[str] = Field(description="错误信息")
    executed_by: Optional[str] = Field(description="操作者")
...
class OperationLogListResponse(OperationLogResponse):
    """操作日志列表响应（简化版）"""
    # 隐藏详细输出信息以提高性能
    output_received: Optional[str] = Field(default=None, description="设备输出（截断）")
    parsed_output: Optional[Dict[str, Any]] = Field(default=None, description="解析后输出（简化）")


class OperationLogDetailResponse(OperationLogResponse):
    """操作日志详情响应"""
    # 包含完整的输出信息和相关上下文
    execution_duration: Optional[float] = Field(default=None, description="执行时长（秒）")
    command_count: Optional[int] = Field(default=None, description="命令数量")
    output_size: Optional[int] = Field(default=None, description="输出大小（字节）")
...
class OperationLogQueryParams(BaseQueryParams, TimeRangeQuery):
    ...
    template_id: Optional[UUID] = Field(default=None, description="按模板ID筛选")
    status: Optional[OperationStatusEnum] = Field(default=None, description="按操作状态筛选")
    executed_by: Optional[str] = Field(default=None, description="按操作者筛选")
    ...
    command_contains: Optional[str] = Field(default=None, description="命令包含关键词")
...
class OperationLogExportRequest(BaseCreateSchema):
    ...
    status: Optional[OperationStatusEnum] = Field(default=None, description="操作状态")
    executed_by: Optional[str] = Field(default=None, description="操作者")
...
class OperationLogBatchDeleteRequest(BaseCreateSchema):
    """批量删除操作日志请求"""
    log_ids: Optional[List[UUID]] = Field(default=None, description="指定日志ID列表")
    before_date: Optional[datetime] = Field(default=None, description="删除此日期之前的日志")
    status: Optional[OperationStatusEnum] = Field(default=None, description="按状态删除")
    device_ids: Optional[List[UUID]] = Field(default=None, description="按设备删除")
    keep_latest_count: Optional[int] = Field(default=None, ge=0, description="每设备保留最新N条")
