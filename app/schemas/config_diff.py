...
class ConfigDiffUpdateRequest(BaseUpdateSchema):
    """配置差异更新请求"""
    diff_content: Optional[str] = Field(default=None, min_length=1, description="差异内容")
    added_lines: Optional[int] = Field(default=None, ge=0, description="新增行数")
    removed_lines: Optional[int] = Field(default=None, ge=0, description="删除行数")
...
class ConfigDiffResponse(BaseResponseSchema):
    ...
    # 关联快照信息
    before_snapshot_type: Optional[str] = Field(default=None, description="变更前快照类型")
    after_snapshot_type: Optional[str] = Field(default=None, description="变更后快照类型")
    ...
    # 差异统计
    total_changes: Optional[int] = Field(default=None, description="总变更行数")
    change_percentage: Optional[float] = Field(default=None, description="变更百分比")
    diff_size: Optional[int] = Field(default=None, description="差异内容大小（字节）")
...
class ConfigDiffListResponse(BaseResponseSchema):
    ...
    # 隐藏详细差异内容以提高性能
    diff_preview: Optional[str] = Field(default=None, description="差异预览（前200字符）")


class ConfigDiffDetailResponse(ConfigDiffResponse):
    """配置差异详情响应"""
    # 包含差异分析结果
    change_summary: Optional[Dict[str, Any]] = Field(default=None, description="变更摘要")
    affected_sections: Optional[List[Dict[str, Any]]] = Field(default=None, description="影响的配置段落")
    risk_analysis: Optional[Dict[str, Any]] = Field(default=None, description="风险分析")


class ConfigDiffQueryParams(BaseQueryParams, TimeRangeQuery):
    """配置差异查询参数"""
    before_snapshot_id: Optional[UUID] = Field(default=None, description="按变更前快照ID筛选")
    after_snapshot_id: Optional[UUID] = Field(default=None, description="按变更后快照ID筛选")
    ...
    added_lines_min: Optional[int] = Field(default=None, ge=0, description="最小新增行数")
    added_lines_max: Optional[int] = Field(default=None, ge=0, description="最大新增行数")
    removed_lines_min: Optional[int] = Field(default=None, ge=0, description="最小删除行数")
    removed_lines_max: Optional[int] = Field(default=None, ge=0, description="最大删除行数")
    has_significant_changes: Optional[bool] = Field(default=None, description="是否有重大变更")
...
class ConfigDiffAnalysisResponse(BaseResponseSchema):
    ...
    security_impact: Optional[Dict[str, Any]] = Field(default=None, description="安全影响")
    
    # 建议
    recommendations: Optional[List[Dict[str, Any]]] = Field(default=None, description="优化建议")
    rollback_complexity: Optional[str] = Field(default=None, description="回滚复杂度")
...
class ConfigDiffStatsResponse(BaseResponseSchema):
    ...
    most_changed_device: Optional[Dict[str, Any]] = Field(default=None, description="变更最多的设备")
